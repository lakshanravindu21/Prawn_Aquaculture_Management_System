...
import jsPDF from 'jspdf'; // Added for professional reporting
import { 
...
export default function Dashboard({ user, onLogout }) {
  ...
  const handleDownloadReport = () => {
    ...
    doc.setFontSize(10).setFont('helvetica', 'normal').text(`Report ID: AS-${Date.now().toString().slice(-6)} | Pond: ${selectedPond}`, 15, 30);
    doc.text(`Generated by: ${user?.name || 'Authorized Researcher'}`, pageWidth - 15, 30, { align: 'right' });

    // Weather Context
    doc.setTextColor(0, 0, 0).setFontSize(14).setFont('helvetica', 'bold').text("1. Local Climate Snapshot (Colombo, LK)", 15, 55);
    doc.setFontSize(10).setFont('helvetica', 'normal').text(`Temperature: ${weather.temp}°C | Condition: ${weather.condition}`, 15, 63);
    ...
    const rows = [
      { n: "Dissolved Oxygen", v: `${latest.dissolvedOxygen} mg/L`, s: (latest.dissolvedOxygen < 4) ? "CRITICAL" : "Optimal" },
      { n: "pH Level", v: `${latest.ph} pH`, s: (latest.ph < 7 || latest.ph > 8.5) ? "Warning" : "Optimal" },
      { n: "Ammonia", v: `${latest.ammonia} mg/L`, s: (latest.ammonia > 0.05) ? "TOXIC" : "Safe" },
    ...
    // Footer
    doc.setFontSize(8).setTextColor(150).text("Generated by AquaSmart Intelligent Framework", pageWidth / 2, 285, { align: 'center' });
  ...
  const calculateSoftSensors = (data) => {
    return data.map(r => {
        ...
        // 1. Updated DO Logic: Adjusted to ensure positive values even at high temp extremes
        let finalDO = r.dissolvedOxygen;
        if (!finalDO || finalDO === 0 || finalDO === "0.00") {
            let calculated = 14.6 - (0.33 * temp) - (0.05 * sal);
            finalDO = Math.max(0.5, calculated).toFixed(2);
        }
            ...
            <div className="space-y-6 flex flex-col">
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="relative group cursor-pointer" onClick={scrollToAlerts}>
                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-white dark:bg-slate-900 border border-indigo-200 dark:border-slate-700 shadow-sm group-hover:bg-rose-50 transition-colors">
                      <BellRing className={`w-4 h-4 ${alerts.length > 0 ? 'text-rose-500' : 'text-slate-400'}`} />
              ...
              <div ref={alertsRef} className="bg-rose-50 dark:bg-slate-800 p-5 rounded-2xl border border-rose-100 dark:border-slate-700 shadow-sm transition-colors duration-300 flex-1 min-h-[300px]">
                ...
                <div className="space-y-3 overflow-y-auto max-h-[300px] pr-1 custom-scrollbar">
                  ...
                  {alerts.length === 0 && <div className="text-center py-8 text-xs text-slate-400 italic">No active alerts. System nominal.</div>}
                </div>
              </div>
            </div>
          </div>
